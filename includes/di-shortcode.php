			...
			if($terms) {
				...
				foreach($terms as $term ) {
					...
					if ($term->parent =='0'){
						...
						print '<li class="docit-main-header"><a href="'.get_home_url().'/'.$di_root_slug.'/'.$tax_slug.'/'.$term->slug.'" class="docit-main-cat-title">'.$term->name.'</a><div><span class="docit-icon-arrowdown"></span></div></li>';
					...
					foreach ($termchildren as $termchild) {
						...
						if(!$subcategoryCheck) {
							...
							$output_sub_posts .= '<li class="docit-sub-header '.$child_term->slug.'"><a href="'.get_home_url().'/'.$di_root_slug.'/'.$tax_slug.'/'.$child_term->slug.'" class="docit-sub-cat-title">'.$child_term->name.'</a><div><span class="docit-icon-arrowdown"></span></div></li>';
						}
						...
						if(!in_array($child_term->term_id,$check_list)) {
							...
							while ($posts->have_posts()) {
								...
								if (!in_array($posts->post->ID,$child_check)){
									$output_sub_posts .= '<li class="docit-sub-post '.$posts->post->post_name.'" id="'.get_the_ID().'"><a href="'.get_home_url().'/'.$di_root_slug.'/'.$posts->post->post_name.'" class="docit-post-title">'. $posts-> post-> post_title .'</a></li>';
								}
								$posts_count = 0;
							...
							foreach ($termchildren_lvl2 as $subkey => $subvalue) {
								...
								if ( $sub_posts-> have_posts() ) {
									if(!in_array($child_of_child_term->term_id,$check_list_lvl2)){
										$output  = ''; 
										$output .= '<li class="docit-sub-sub-header '.$child_of_child_term->slug.'"><a href="'.get_home_url().'/'.$di_root_slug.'/'.$tax_slug.'/'.$child_of_child_term->slug.'" class="docit-sub-sub-cat-title">'. $child_of_child_term->name .'</a></li>';
									}
									//loop through posts
									while ( $sub_posts-> have_posts() ) {
										...
										$sub_posts-> the_post();
										// show post titles for this cat
										$output_sub_posts .= '<li class="docit-sub-sub-post '.$sub_posts->post->post_name.'"><a href="'.get_home_url().'/'.$di_root_slug.'/'.$sub_posts->post->post_name.'" class="docit-post-title">'. $sub_posts->post->post_title .'</a></li>';
					...
					if ( $posts-> have_posts() ) {
						...
						while ( $posts-> have_posts() ) {
							...
							// show post titles for this cat
							print '<li class="docit-main-link '.$posts->post->post_name.'"><a href="'.get_home_url().'/'.$di_root_slug.'/'.$posts->post->post_name.'" class="docit-post-title">'. $posts-> post-> post_title .'</a></li>';
